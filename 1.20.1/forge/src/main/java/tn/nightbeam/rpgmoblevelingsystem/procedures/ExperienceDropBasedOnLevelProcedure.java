...
public class ExperienceDropBasedOnLevelProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if ((entity instanceof LivingEntity _livingEntity0 && _livingEntity0.getAttributes().hasAttribute(RpgMobLevelingSystemModAttributes.AMLEVEL_ATTRIBUTE.get())
				? _livingEntity0.getAttribute(RpgMobLevelingSystemModAttributes.AMLEVEL_ATTRIBUTE.get()).getValue()
				: 0) > 0) {
			if (!JaumlConfigLib.getBooleanValue("RpgMobLevelingSystem", "mobs_list_settings", "death_messages")) {
			...
			if (world instanceof ServerLevel _level)
				_level.addFreshEntity(new ExperienceOrb(_level, x, y, z,
						(int) Math.floor((entity instanceof LivingEntity _livingEntity4 && _livingEntity4.getAttributes().hasAttribute(RpgMobLevelingSystemModAttributes.AMLEVEL_ATTRIBUTE.get())
								? _livingEntity4.getAttribute(RpgMobLevelingSystemModAttributes.AMLEVEL_ATTRIBUTE.get()).getValue()
								: 0) * JaumlConfigLib.getNumberValue("RpgMobLevelingSystem", "scale_settings", "xp_modfier"))));
		}
	}
}