...
public class CreateOrReplaceGlobalConfigProcedure {
	...
	private static void execute(@Nullable Event event) {
		...
		if (!JaumlConfigLib.arrayKeyExists(dir, filename, "overlay_x_offset")) {
			JaumlConfigLib.setNumberValue(dir, filename, "overlay_x_offset", 0);
		}
		if (!JaumlConfigLib.arrayKeyExists(dir, filename, "overlay_y_offset")) {
			JaumlConfigLib.setNumberValue(dir, filename, "overlay_y_offset", 0.45);
		}
		if (!JaumlConfigLib.arrayKeyExists(dir, filename, "overlay_z_offset")) {
			JaumlConfigLib.setNumberValue(dir, filename, "overlay_z_offset", 0);
		}
	}
}