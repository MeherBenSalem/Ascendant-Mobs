...
public class CalculateSpawnDifferenceProcedure {
	public static double execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		String scaletype = "";
		scaletype = JaumlConfigLib.getStringValue("RpgMobLevelingSystem", "scale_settings", "scale_type");
		if (scaletype.contains("motp")) {
			...
			if ((scaletype).equals("vertical")) {
				...
				return Math.floor((world.dayTime() / 24000) / JaumlConfigLib.getNumberValue("RpgMobLevelingSystem", "scale_settings", "day_factor"));
		...
		return Math.floor(spawnlocation / JaumlConfigLib.getNumberValue("RpgMobLevelingSystem", "scale_settings", "scale_distance"));
	}
}