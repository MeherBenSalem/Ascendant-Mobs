...
public final class MobLevelService {
    ...
    public static void onMobDeath(LivingEntity living) {
        ...
        for (ModConfig.LootBand band : ModConfig.lootByLevel().bands) {
            ...
            var itemKey = net.minecraft.resources.Identifier.tryParse(band.itemId);
            if (itemKey == null) {
                continue;
            }
            Item item = BuiltInRegistries.ITEM.get(itemKey).map(reference -> reference.value()).orElse(null);
            if (item == null) {
                continue;
            }
    ...
    private static void applyMobLevel(Level world, Entity entity, boolean force) {
        ...
        Constants.LOG.info("Applied level {} to {}", (int) Math.floor(level), EntityClassification.entityTypeId(entity));
    }

    private static boolean shouldForceRelevel(Entity entity) {
        if (entity.getType() == EntityTypes.SLIME && MobLevelStorage.hasBeenLeveled(entity)) {
