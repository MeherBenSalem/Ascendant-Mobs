...
public final class LevelCalculationService {
    ...
    public static double resolveStructureMinLevel(Level world, Entity entity) {
        ...
        for (var entry : ModConfig.mobs().structureMinLevels.entrySet()) {
            Identifier structureId = Identifier.tryParse(entry.getKey());
            if (structureId == null) {
                continue;
            }
            var structureHolder = registry.get(ResourceKey.create(Registries.STRUCTURE, structureId));
            Structure structure = structureHolder.map(holder -> holder.value()).orElse(null);
            if (structure == null) {
                continue;
            }
