/**
 * Upload local dist/ jars to Modrinth + CurseForge (no GitHub Actions).
 * Tokens: env vars or NightBeam-Knowledge-Base/secrets/local.env
 *
 * Usage:
 *   node scripts/upload_platforms.mjs --version 2.0.2
 *   node scripts/upload_platforms.mjs --version 2.0.2 --curseforge-only
 */
import fs from "fs";
...
const MODRINTH_ID = "4eSfhd3M";
const CURSEFORGE_ID = "1088344";
const MOD_TITLE = "RPG Mob Leveling System";
...
async function main() {
  ...
  const changelog = args.changelogFile && fs.existsSync(args.changelogFile)
    ? fs.readFileSync(args.changelogFile, "utf8")
    : `## ${MOD_TITLE} ${version}\n\nSee GitHub release notes.`;

  const jars = fs
    .readdirSync(DIST)
    .filter((f) => f.startsWith("rpgmoblevelingsystem-") && f.endsWith(`-${version}.jar`))
    .map((f) => path.join(DIST, f))
    .sort();
  if (!jars.length) throw new Error(`No rpgmoblevelingsystem-*-${version}.jar in dist/`);
  ...
  console.log(`Uploading ${MOD_TITLE} ${version} (${jars.length} jars) → Modrinth ${MODRINTH_ID}, CF ${CURSEFORGE_ID}`);

  if (!args.curseforgeOnly) {
    for (const p of parsed) {
      const body = {
        name: `${MOD_TITLE} ${version}`,
      ...
      if (args.dryRun) {
        console.log("[dry-run] Modrinth", p.name, body);
        continue;
      }
